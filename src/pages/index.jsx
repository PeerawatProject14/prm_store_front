...
export default function HotIssuePage() {
  ...
  const moduleAlertedRef = useRef(false);
  ...
  useEffect(() => {
    ...
    const checkAccess = async () => {
      try {
        ...
        if (!enabled) {
          if (!moduleAlertedRef.current) {
            moduleAlertedRef.current = true;
        ...
        // ✅ ถ้าเป็นปัญหา auth ให้ปล่อยให้ api.js จัดการ redirect เอง
        const status = err?.response?.status;
        const code = err?.code;

        if (status === 401 || code === "NO_TOKEN") {
          // ไม่ทำอะไรเพิ่ม ลดโอกาส ping-pong
          return;
        }
        ...
        if (alive) setCheckingModule(false);
      }
    };
  ...
  const combinedDoneByType = useMemo(() => {
    ...
    (cases || []).forEach((c) => {
      if (c.status_code === "DONE" || c.status_code === "CLOSED") {
        if (c.type_code === "HOT_ISSUE" || c.type_code === "HOT")
          result.HOT_ISSUE += 1;
  ...
  const { summaryData, typeWeekly, totalStats } = useMemo(() => {
    ...
    const tStats = {
      total: (cases || []).length,
        ...
        (c) => c.status_code === "DONE" || c.status_code === "CLOSED"
      ).length,
      label: "All Time",
    };
